...
public class CommentServiceImpl implements CommentService {
    ...
    public DataResponse createComment(CommentRequestDTO.Create create, String email,CommentType commentType) {
        ...
        if(commentType == CommentType.TRANSACTION){
            ...
            TransactionComment transactionComment = TransactionComment.toEntity(create.getContent(), getUser, getTransactionPost);
            transactionCommentRepository.save(transactionComment);
        }else if(commentType == CommentType.OPINION){
            OpinionPost getOpinionPost = opinionPostRepository.findById(create.getPostId()).orElseThrow(()-> new NullPointerException("Invalid argument"));
            ...
            VotePost getVotePost = votePostRepository.findById(create.getPostId()).orElseThrow(() -> new NullPointerException("Invalid argument"));
    ...
    public DataResponse updateComment(CommentRequestDTO.Update update, String email,CommentType commentType) {
        ...
        if(commentType == CommentType.TRANSACTION){
            ...
            OpinionComment getOpinionComment = opinionCommentRepository.findById(update.getCommentId()).orElseThrow(() -> new NullPointerException("Invalid argument"));
            ...
            VoteComment getVoteComment = voteCommentRepository.findById(update.getCommentId()).orElseThrow(() -> new NullPointerException("Invalid argument"));
